...
def apkpureWebScrapper(url, package_name):
    ...
    global webScrapper, console, downloadLinksJSON
    ...
    while retry_count < max_retries:
        try:
            response = webScrapper.get(f"{url}/search?q=" + package_name)
    ...
    response = webScrapper.get(url + package_uri + "/versions").text
    ...
    try:
        for href_link in ver_download_link:
            ...
            if href_link is not None:
                download_page = webScrapper.get(href_link).text
                if "Download Variant" in download_page:
                    ...
                    for variant in variants_uris:
                        variant_download_page = webScrapper.get(url + variant).text
                        ...
                        if not any(entry["download_url"] == variant_download_url for entry in applications): # In some cases, it appears that the apkpure provides the same APK file for all variants (?)
...
def main():
    ...
    global webScrapper, console, apiScrapper, downloadLinksJSON, packageName
    
    webScrapper = cfscrape.create_scraper(delay=10)
    ...
    if os.path.exists(downloadLinksJSON) and os.path.getsize(downloadLinksJSON) != 2:
        ...
        with console.status("[bold green]Scrapping...") as status:
            ...
            try:
                ...
                try:
                    response = requests.get(web_url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/113.0"})
    ...
    if args.nd:
        ...
        try:
            ...
            print("\n[!] Keyboard interruption detected while downloading, we'll shutdown execution after running threads are finished.")
            wait(2)
            print("... Promise!")
